    <div class="contenedor_cuerpo" name="dv_regpro" id="dv_regpro">       
        <fieldset>
	    <p class="titulo_modulo">REPORTE DE ASISTENCIA DOCENTE</p>
            <p style="font-weight: bold">Fecha de Asistencia:
	    <input class="detalle_" type="text" name="i_fecha_log" placeholder="Seleccione fecha" id="i_fecha_log" style="width:180px; height:30px;; border: 1px solid rgb(172, 25, 61);"> 
            </p>
		...
		<br/><br/><button class="css_btn" id="btn_asigCarga" onclick="generarReporte()">GENERAR REPORTE</button>
		
		<select class="detalle_" name="cmb_carrera" id="cmb_carrera" style="width:200px;display: none" disabled>
                    <option value="0">CARRERA</option>               
                </select>
		
                <select class="detalle_" name="cmb_ciclo" id="cmb_ciclo" style="width:200px; display: none" disabled>
                ...
                <select class="detalle_" name="cmb_salon" id="cmb_salon" style="width:200px; display: none" onchange="cargar_horario_salon();ocultar();cargar_docentes()" disabled>
                   <option value="0">SALON</option>                   
                </select>
                <br><br>

                <select class="detalle_" name="cmb_cursoxsal_u" id="cmb_cursoxsal_u" style="width:350px; display: none" disabled="true">
...
<script>
        function generarReporte() {
            var local =$("#cmb_facultad").val();
	    var fechamarcado = $("#i_fecha_log").val();
	    
	    if (local=='0' || fechamarcado=='') {
		alerta("Debe seleccionar la fecha y la facultad","Error");
	    }
	    else{
		    var anio=fechamarcado.substring(6, 10);
		    var mes = fechamarcado.substring(3, 5);
		    var dia = fechamarcado.substring(0, 2);
		    
		    fechamarcado=anio+'-'+mes+'-'+dia;
	
		    
		    $.post("rpt_AsistenciaDocente", {local:local,fechamarcado:fechamarcado} ,function(resultado)
				    {
					if(resultado != false)
					{
					    $("#AsistenciaDocente").empty();
					    $("#AsistenciaDocente").append(resultado);
					}
				    }); 
	
	    
	    
		    $('#AsistenciaDocente').dialog({
