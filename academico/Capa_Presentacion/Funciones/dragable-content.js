	...
	*/	
	...
	function initDragDropElement(e)
	{
		...
		if(!dragableElementMoved[numericIdToBeDragged]){
			if(primerpase)
			{
				dragObjCloneArray[numericIdToBeDragged].style.top = getTopPos(dragObjArray[numericIdToBeDragged])-35 + 'px';
				primerpase=false;
			}
			else
				dragObjCloneArray[numericIdToBeDragged].style.top = getTopPos(dragObjArray[numericIdToBeDragged])-dragObjCloneArray[numericIdToBeDragged].style.top + 'px';
	...
	function timerDragDropElement()
	{
		...
		if(dragDropTimer>=10){
			if(dragObjCloneArray[numericIdToBeDragged].style.display=='none'){
				...
				dragObjCloneArray[numericIdToBeDragged].style.top = 0;//getTopPos(dragObjArray[numericIdToBeDragged]) + 'px';
				//dragObjCloneArray[numericIdToBeDragged].style.left = getLeftPos(dragObjArray[numericIdToBeDragged]) + 'px';
	...
	{		
		...
		var altodelaimagen=parseInt($("#altobannerimg").val());
		var anchodelaimagen=parseInt($("#anchobannerimg").val());
                       
		var altodelaimagen_act=parseInt($("#banerimagen").height());
		var anchodelaimagen_act=parseInt($("#banerimagen").width());

		dragObjCloneArray[numericIdToBeDragged].style.top = (e.clientY - mouse_y + el_y) + 'px'; 

		if(altodelaimagen<=390 && anchodelaimagen>=900)
	        dragObjCloneArray[numericIdToBeDragged].style.top=0;
	    else if(altodelaimagen>390 && anchodelaimagen<900)
	    {
	    		if((e.clientY - mouse_y + el_y)>0)
			{
				dragObjCloneArray[numericIdToBeDragged].style.top=0;
			}		
		
			if(altodelaimagen_act>=600)
				var topeabajo=(altodelaimagen_act*0.395)*-1;
			else
				var topeabajo=(altodelaimagen_act*0.22)*-1;

			if((e.clientY - mouse_y + el_y)<topeabajo)
			{
				dragObjCloneArray[numericIdToBeDragged].style.top=topeabajo;
			};
	    }
	    else if(altodelaimagen<=390 && anchodelaimagen<=900)
	        dragObjCloneArray[numericIdToBeDragged].style.top=0;
	    else if(altodelaimagen>390 && altodelaimagen< (anchodelaimagen/2) && anchodelaimagen>900)
	        dragObjCloneArray[numericIdToBeDragged].style.top=0;
	    else if(anchodelaimagen>(anchodelaimagen/3) && anchodelaimagen>900)
	    {
	    	if((e.clientY - mouse_y + el_y)>0)
			{
				dragObjCloneArray[numericIdToBeDragged].style.top=0;
