<form id="form_buscar" method="GET" action="fseco-adm">
	<input type="hidden" name = "codBuscar" id= "codBuscar" value="" />
</form>
<script type="text/javascript">
  $(function(){
//al momento de cargar pone de color resaltado a la pestaña de Dts.Pers
    $("#per").css({
      "background": "#f5c774"
    });
//fin al momento de cargar pone de color resaltado a la pestaña de Dts.Pers
    limpiar();//limpiar todas las casillas
    //cargar_lugar();
    //cargar_lugar_colegio();
    <?php
    	$codBuscar="";
      	if(isset($_GET['codBuscar'])){
      		$codBuscar=$_GET['codBuscar'];
      	}
      	else
      	{
      		$codBuscar="";
      	}
      $param="<Dato><x_codigo>".$codBuscar."</x_codigo></Dato>";
    ...
    ?>
    //capturamos el codigo si eso encuesta
...
<div class="contenedor_cuerpo" style="padding-top: 4px;">    
    ...
    <?php set_imagen("fseco/error.png","15","15");?>, Incompleto
		<!--botones de imprimir y guardar -->
	      <div align="right">
  			<label id="lab_bus">CODIGO</label>
          	<input type="text" id="txtCodigo_p" name="txtCodigo_p" maxlength="7" <?php if(isset($_GET['codBuscar'])) echo 'value="'.$_GET['codBuscar'].'"'?>  />

          	<button title="Buscar"  type="button" id="cod_b" value="Datos Conocidos" onclick=buscar($("#txtCodigo_p").val());><?php set_imagen("fseco/icono.jpg","30","30");?></button>
          	<button title="Imprimir"  type="button" id="cod_b" value="Datos Conocidos" onclick=imprimir($("#txtCodigo_p").val(),"adm_fseco"); ><?php set_imagen("fseco/icono_imp.jpg","30","30");?></button>   
          ...
          <br>
        <Script>
	        $('#txtCodigo_p').bind('keyup', function (e) {
	          var key = e.keyCode || e.which;
	          if (key === 13) {
	            buscar($("#txtCodigo_p").val());
	          };
	        });      
        </Script>
          	...
          	<div id="steps">
	            ...
	            <div id="mensaje_correcto" style="display: none;" title="Correcto">
	                    <!--<div style="width:450px; height: 190px;" id="int_dialog" align="center">
	                        <tr align="center">
                          <?php set_imagen("fseco/correcto.jpg","150","150");?>
	                         
	                        </tr>
	                    </div>-->
	           </div>
	                  <input type="hidden" name = "action" id= "action" value="guardarFSECO" />
	                  <input type="hidden" name = "txtCodigoAlumno" id= "txtCodigoAlumno" value="" />
	                  <input type="hidden" name = "usuAdmin" id= "usuAdmin" value="admin" />
